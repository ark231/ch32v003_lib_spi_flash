...
void is25x_end_dma_read_p(volatile IS25x *self);  // private, do not call from user code

// write to is25x without dma, blocking until finished
void is25x_write_no_dma(IS25x *self, uint32_t addr, uint8_t *src, size_t len);

// nonblocking write operation using dma
void is25x_begin_dma_write(IS25x *self, uint32_t addr, uint8_t *src, size_t len);
bool is25x_dma_write_is_completed(IS25x *self);
void is25x_end_dma_write_p(volatile IS25x *self);  // private, do not call from user code
